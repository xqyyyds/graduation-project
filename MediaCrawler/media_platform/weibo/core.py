...
from var import crawler_type_var, hot_trend_id_var, source_keyword_var
...
class WeiboCrawler(AbstractCrawler):
        ...
        async with async_playwright() as playwright:
            ...
            if not await self.wb_client.pong():
                ...
                await self.wb_client.update_cookies(
                    browser_context=self.browser_context, urls=[self.mobile_index_url]
                )
    ...
    async def search(self):
        ...
        keyword_to_hot_trend_id = getattr(config, "HOT_TREND_ID_BY_KEYWORD", {}) or {}

        for keyword in config.KEYWORDS.split(","):
            keyword = keyword.strip()
            source_keyword_var.set(keyword)
            # IMPORTANT: hot_trend_id is metadata only; never append it to keyword.
            hot_trend_id_var.set(keyword_to_hot_trend_id.get(keyword))
            utils.logger.info(
                f"[WeiboCrawler.search] Current search keyword: {keyword}"
            )

            max_notes_for_keyword = int(
                getattr(config, "CRAWLER_MAX_NOTES_COUNT", 0) or 0
            )
            if max_notes_for_keyword <= 0:
                utils.logger.info(
                    f"[WeiboCrawler.search] CRAWLER_MAX_NOTES_COUNT={max_notes_for_keyword}, skip keyword: {keyword}"
                )
                continue

            processed_notes = 0
            page = start_page
            while processed_notes < max_notes_for_keyword:
                utils.logger.info(
                ...
                # If returned ok:0 and indicates no content, stop paging
                if isinstance(search_res, dict) and search_res.get("ok") == 0:
                    msg = search_res.get("msg", "") or ""
                    if "这里还没有内容" in msg or (
                        isinstance(search_res.get("data"), dict)
                        and search_res["data"].get("cards") == []
                    ):
                        utils.logger.info(
                            f"[WeiboCrawler.search] no content on page {page} for keyword {keyword}, stop paging"
                        )
                        break
                ...
                if not note_list:
                    utils.logger.info(
                        f"[WeiboCrawler.search] empty note list on page {page} for keyword {keyword}, stop paging"
                    )
                    break
                ...
                remaining = max_notes_for_keyword - processed_notes
                if remaining <= 0:
                    break
                if len(note_id_list) > remaining:
                    note_id_list = note_id_list[:remaining]
                # fetch full note details concurrently and save (to retrieve full text)
                if note_id_list:
                    sem = asyncio.Semaphore(config.MAX_CONCURRENCY_NUM)
                    tasks = [
                        asyncio.create_task(self.get_note_info_task(nid, sem))
                        for nid in note_id_list
                    ]
                    note_details = await asyncio.gather(*tasks)

                    # 注意：这里原来是逐条 await（保存 + 下载图片），会把前面并发取详情的收益全部“串行化”掉。
                    valid_details = [d for d in note_details if d]
                    if valid_details:
                        await asyncio.gather(
                            *(
                                weibo_store.update_weibo_note(note_detail)
                                for note_detail in valid_details
                            )
                        )
                        await asyncio.gather(
                            *(
                                self.get_note_images(note_detail.get("mblog"))
                                for note_detail in valid_details
                                if note_detail.get("mblog")
                            )
                        )

                    processed_notes += len(note_id_list)
                ...
                utils.logger.info(
                    f"[WeiboCrawler.search] Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds after page {page-1}"
                )

                await self.batch_get_notes_comments(note_id_list)

                if processed_notes >= max_notes_for_keyword:
                    utils.logger.info(
                        f"[WeiboCrawler.search] reached max notes ({max_notes_for_keyword}) for keyword {keyword}, stop"
                    )
                    break
    ...
    async def get_specified_notes(self):
        ...
        valid_details = [d for d in video_details if d]
        if valid_details:
            await asyncio.gather(
                *(
                    weibo_store.update_weibo_note(note_item)
                    for note_item in valid_details
                )
            )
    ...
    async def get_note_info_task(
        ...
        try:
            # 只把真正的网络请求放在 semaphore 里，避免 sleep 占用并发名额导致看起来“没并发”
            async with semaphore:
                result = await self.wb_client.get_note_info_by_id(note_id)
        except DataFetchError as ex:
            utils.logger.error(
                f"[WeiboCrawler.get_note_info_task] Get note detail error: {type(ex).__name__}: {ex}"
            )
            # 以 DEBUG 级别记录完整回溯，便于后续排查
            utils.logger.debug("Get_note_info_task exception traceback:", exc_info=True)
            result = None
        ...
        # Sleep after fetching note details（放到 semaphore 外，避免占用并发名额）
        await asyncio.sleep(config.CRAWLER_MAX_SLEEP_SEC)
        utils.logger.info(
            f"[WeiboCrawler.get_note_info_task] Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds after fetching note details {note_id}"
        )
    ...
    async def get_note_comments(self, note_id: str, semaphore: asyncio.Semaphore):
        ...
        try:
            ...
            # Sleep before fetching comments（放到 semaphore 外，避免占用并发名额）
            await asyncio.sleep(config.CRAWLER_MAX_SLEEP_SEC)
            utils.logger.info(
                f"[WeiboCrawler.get_note_comments] Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds before fetching comments for note {note_id}"
            )
    ...
    async def get_note_images(self, mblog: Dict):
        ...
        for pic in pics:
            ...
            await asyncio.sleep(config.CRAWLER_MAX_SLEEP_SEC)
            utils.logger.info(
                f"[WeiboCrawler.get_note_images] Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds after fetching image"
            )
            if content != None:
    ...
    async def create_weibo_client(self, httpx_proxy: Optional[str]) -> WeiboClient:
        ...
        cookie_str, cookie_dict = utils.convert_cookies(
            await self.browser_context.cookies(urls=[self.mobile_index_url])
        )
