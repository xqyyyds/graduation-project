    ...
    async def store_content(self, content_item: Dict):
        ...
        async with get_session() as session:
            ...
            if db_note:
                ...
                db_note.last_collected = now_ts
                db_note.seen_count = (db_note.seen_count or 0) + 1
                ...
                content_item["first_collected"] = now_ts
                content_item["last_collected"] = now_ts
                content_item["seen_count"] = 1
    ...
    async def store_comment(self, comment_item: Dict):
        ...
        async with get_session() as session:
            ...
            if db_comment:
                ...
                db_comment.last_collected = now_ts
                db_comment.seen_count = (db_comment.seen_count or 0) + 1
                ...
                comment_item["first_collected"] = now_ts
                comment_item["last_collected"] = now_ts
                comment_item["seen_count"] = 1
...
class WeiboMongoStoreImplement(AbstractStore):
    ...
    def __init__(self):
        ...
        try:
            asyncio.create_task(
                self.mongo_store.create_index("contents", [("note_id", 1)], unique=True)
            )
            asyncio.create_task(
                self.mongo_store.create_index(
                    "comments", [("comment_id", 1)], unique=True
                )
            )
            asyncio.create_task(
                self.mongo_store.create_index(
                    "contents", [("hot_trend_id", 1)], unique=False
                )
            )
            asyncio.create_task(
                self.mongo_store.create_index(
                    "comments", [("hot_trend_id", 1)], unique=False
                )
            )
        except Exception:
            # 在某些测试环境中 create_task 可能不可用，忽略索引创建错误
            pass
    ...
    async def store_content(self, content_item: Dict):
        ...
        await self.mongo_store.upsert_with_meta(
