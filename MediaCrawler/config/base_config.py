...
# 数据保存类型选项配置,支持：csv、db、json、sqlite、excel、mongodb。此处改为 mongodb。
...
# ---- Weibo request & IPv6 control ----
# 单次 HTTP 请求超时时间（秒），用于 httpx .request 的 timeout 参数
WEIBO_REQUEST_TIMEOUT_SEC = 3
# 单次请求的总超时（秒），包含主尝试与回退尝试
WEIBO_REQUEST_TOTAL_TIMEOUT_SEC = 15
# IPv6 池每个 IP 的最大并发（避免单IP并发过大）
IPV6_PER_IP_CONCURRENCY = 1
