...
MONGODB_DB_NAME = os.getenv("MONGODB_DB_NAME", "media_crawler_db")
...
def build_mongo_uri(
    ...
    port: int | str = MONGODB_PORT,
    user: str = MONGODB_USER,
    password: str = MONGODB_PWD,
    db_name: str = MONGODB_DB_NAME,
) -> str:
    """Construct a MongoDB URI from config values; includes auth if user/password provided."""
    port_int = int(port)
    if user and password:
        return f"mongodb://{user}:{password}@{host}:{port_int}/{db_name}"
    return f"mongodb://{host}:{port_int}/{db_name}"


# Default URI for scripts and stores
