...
def check_db_status():
    ...
    try:
        ...
        logger.error(f"⚠️ 无法获取数量 (可能是连接问题): {e}")
        return
    ...
    for test_query in test_queries:
        logger.info("\n" + "-" * 70)
        logger.info(f"❓ 模拟用户提问: “{test_query}”")
        ...
        for i, doc in enumerate(results, start=1):
            ...
            logger.info(f"   🏷️ 类别: {doc.metadata.get('category')}")
            logger.info(f"   ⚠️ 风险: {doc.metadata.get('risk_level')}")
